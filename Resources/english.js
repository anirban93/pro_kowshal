exports.eng = function(){
	
	var win_eng= Ti.UI.createWindow({
		backgroundImage: "/images/b2.jpg"
	});
	
	var dataEng= Ti.Database.install('/datbs/engy.db','engy');
...
about_img.add(close_button);
	...
	//var m;
 	//////////dataEngase Part !!
			...
			var db_eow= dataEng.execute('SELECT ques FROM eng WHERE sets=?', myglo.k+1 );
			var oeti = dataEng.execute('SELECT op1,op2,op3,op4 FROM eng WHERE sets=?', myglo.k+1 );
			...
			{	
				...
				while(db_eow.isValidRow()&&oeti.isValidRow()){
						views[vu] = viw(vu);
						
						
						var db_ans= dataEng.execute('SELECT tr FROM eng WHERE rowid=?', vu+1 );
	...
	win_eng.addEventListener('android:back',function(){
	 	myglo.k=-1;
	 	win_eng.close();
	 }); 
