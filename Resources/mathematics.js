exports.math = function(){
	...
	var win_math= Ti.UI.createWindow({
		backgroundImage: "/images/b2.jpg"
	});
	
	var dataMath= Ti.Database.install('/datbs/maths.db','maths');
...
about_img.add(close_button);
	...
	//var m;
 	//////////dataMathase Part !!
			...
			var db_mow= dataMath.execute('SELECT ques FROM math WHERE sets=?', myglo.k+1 );
			var omti = dataMath.execute('SELECT op1,op2,op3,op4 FROM math WHERE sets=?', myglo.k+1 );
			...
			{	
				...
				while(db_mow.isValidRow()&&omti.isValidRow()){
						views[vu] = viw(vu);
						
						
						var db_ans= dataMath.execute('SELECT tr FROM math WHERE rowid=?', vu+1 );
	...
	win_math.addEventListener('android:back',function(){
	 	myglo.k=-1;
	 	win_math.close();
	 }); 
